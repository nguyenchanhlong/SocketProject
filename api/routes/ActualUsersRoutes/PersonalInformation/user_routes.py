...
def user_routes():
    ...
    def signup():
        ...
        new_user = UserProperty(
            username=username, password=password, nameaccount=nameaccount
        )
    ...
    def login():
        ...
        claims = {"sub": sub, "exp": expiration_time}
        ...
        if user_inf and user_inf.get("password") == password:
            ...
            encoded_data = jwt.encode(
                payload=claims, key=settings.AUTH_SECRET_KEY, algorithm="HS256"
            )
            ...
            user_inf = {
                k: object_id_converter(v) if isinstance(v, ObjectId) else v
                for k, v in user_inf.items()
            }
            return jsonify(
                {
                    "Name Account": user_inf["nameaccount"],
                    "User Name": user_inf["username"],
                    "User Password": user_inf["password"],
                    "Access Token": auth_token_dict["authToken"],
                }
            ), 200
        else:
